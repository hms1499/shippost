...
// Scheduled sweeper (see vercel.json crons). Two passes over "paid, never
// delivered, never refunded":
//   status='pending'  — died silently; queue a slow-cancel refund, flip to 'failed'
//   status='failed'   — failed loudly with no tweets; queue a full refund
// The second pass is what makes the UI's "a refund is queued automatically" true;
// before it, only a user tapping the button ever queued one.
// Never sends money; a human still drains the queue. Vercel Cron authenticates
// with the Authorization: Bearer $CRON_SECRET header it injects when CRON_SECRET
// is set.
export async function GET(req: Request) {
  ...
  try {
    ...
    if (result.swept > 0 || result.enqueuedFailed > 0 || result.errors.length > 0) {
      await alertOps(
